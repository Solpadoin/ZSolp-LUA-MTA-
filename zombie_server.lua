---> Recoded by Solpadoin <---
--== WORLD WAR Z SIMULATION GAMEMODE ==--

ResourceName = getResourceName ( getThisResource() )
ZombieLimit = get(ResourceName..".MaxZombies") -- HOW MANY ZOMBIES SHOULD EXIST AT MAXIMUM? // fixed not working when resource gets rename
ZombieStreaming = get(ResourceName..".StreamMethod") -- 1 to constantly stream zombies, 0 to only allow zombies to spawn via createZombie function, 2 to only allow spawning at set spawnpoints
...
ZombieSpeed = get(ResourceName..".Speed")


--- New Settings by Solpadoin ---

IsDefaultAnims = get(ResourceName..".IsDefaultAnim")
IsBothAnim = get(ResourceName..".IsBothAnim")
AnimFast = get(ResourceName..".AnimFast")
AnimHurt = get(ResourceName..".AnimHurt")
IsZombiesWeak = get(ResourceName..".IsZombiesWeak")
MaxLimitPerPlayer = get(ResourceName..".LimitPerPlayer")

---

---// FIX ME  ---//
if (IsZombiesWeak == 1) then
	triggerClientEvent ( resourceRoot , "Zomb_SetWeakZombies", resourceRoot )
end
---//


if (IsDefaultAnims == 1) then
	...
	elseif ZombieSpeed == 2 then -- rocket zombies (possibly stressful on server)
		chaseanim = "Run_Wuzi"
		checkspeed = 680
	elseif ZombieSpeed == 3 then -- Fight Stile edit by Solp
		chaseanim = "FightShF"
		checkspeed = 680
	elseif ZombieSpeed == 4 then -- Super fast zombies by Solp
...
if (IsBothAnim == 1) then
	if (IsDefaultAnim == 1) then
		chaseanim1 = "run_civi"		--- chaseanim1 -- > FastAnimstate
		checkspeed = 600
	else
		chaseanim1 = AnimFast
		checkspeed = 600
...
--IDLE BEHAVIOUR OF A ZOMBIE ,, recode by Solpadoin
	...
	if isElement(ped) then
		if (isPedDead ( ped ) == false ) then
			...
			elseif action == 6 then
				setPedAnimation ( ped, "WOMAN_walkshop", "ped", -1, false, true, true)
				setTimer ( Zomb_Idle, 4000, 1, ped )
			elseif action == 7 then
				setPedAnimation ( ped, "BED_Out_L", "INT_HOUSE", -1, false, true, true)
				setTimer ( Zomb_Idle, 2300, 1, ped )
			elseif action == 8 then
				setPedAnimation ( ped, "BED_Out_R", "INT_HOUSE", -1, false, true, true)
				setTimer ( Zomb_Idle, 2300, 1, ped )
			end
		end
	end
	...
	if isElement(ped) then
		if (getElementData ( ped, "status" ) == "chasing") and (getElementData (ped, "zombie") == true) then
			...
			if (distance < 1 ) then -- IF THE PED HASNT MOVED
				if (getElementData ( ped, "target" ) == nil) then
					...
					if giveup == 1 then
						...
						if action == 1 then
							...
							setTimer ( function (ped) if ( isElement ( ped ) ) then if (getElementHealth(ped) > 70) then setPedAnimation ( ped, "ped", chaseanim1, -1, true, true, true ) else setPedAnimation ( ped, "ped", chaseanim, -1, true, true, true ) end end end, 800, 1, ped )
				...
				if (getElementHealth(ped) > 70) then setPedAnimation ( ped, "ped", chaseanim1, -1, true, true, true ) else setPedAnimation ( ped, "ped", chaseanim, -1, true, true, true ) end --KEEP WALKING
...
--		call(getResourceFromName("scoreboard"), "scoreboardAddColumn", "Zombie kills") --ADDS TO SCOREBOARD
		...
		if ZombieSpeed == 2 or ZombieSpeed == 4 then
	...
	if safezone == 0 then
		if table.getn ( everyZombie ) < newZombieLimit then
			...
			if zomb ~= false then
				...
				setTimer ( function (zomb) if ( isElement ( zomb ) ) then if (getElementHealth(zomb) > 70) then setPedAnimation ( zomb, "ped", chaseanim1, -1, true, true, true ) else setPedAnimation ( zomb, "ped", chaseanim, -1, true, true, true ) end end end, 1000, 1, zomb )
...
function SpawnZombie ()
	local pacecount = 0
	while pacecount < 4 do	--4 ZOMBIES AT A TIME TO PREVENT FPS DROP
...
function player_Spawn ()
	if ZombieStreaming == 1 or ZombieStreaming == 2 then
		...
		if (gPlayerCount > 10 and relocatecount < gPlayerCount) then
			for ZombKey,theZomb in ipairs(everyZombie) do
				if relocatecount < gPlayerCount then
					if ( getElementData ( theZomb, "forcedtoexist" ) ~= true) then
						if ( getElementData ( theZomb, "target" ) == nil ) and ( isPedDead ( theZomb ) == false ) and (getElementData (theZomb, "zombie") == true) then
							relocatecount = relocatecount+1
							Zomb_delete(theZomb)
						end
					end
...
function FCheckPlayerCount()
	gPlayerCount = getPlayerCount()
end
setTimer (FCheckPlayerCount, 300000, 0)
